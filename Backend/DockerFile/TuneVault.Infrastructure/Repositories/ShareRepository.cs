...
public class ShareRepository : IShareRepository
{
    ...
    public async Task<IEnumerable<MediaShare>> GetSharedWithMeAsync(string receiverId)
    {
        string sql = @"
            SELECT 
                ms.*
            FROM MediaShare ms
            LEFT JOIN MediaItem mi ON mi.MediaItemID = ms.MediaItemID
            LEFT JOIN Playlist p ON p.PlaylistID = ms.PlaylistID
            WHERE ms.ReceiverID = @ReceiverID
            ORDER BY ms.SharedAt DESC";

        return await _db.LoadAllDataSingleAsync<MediaShare>(
            sql,
    ...
    public async Task<bool> AlreadySharedAsync(
        string senderID,
        string receiverID,
        int? mediaItemID,
        int? playlistID)
    {
        string sql = @"
            ...
            WHERE SenderID = @SenderID
              ...
              AND (
                    (@MediaItemID IS NOT NULL AND MediaItemID = @MediaItemID)
                    OR
                    (@PlaylistID IS NOT NULL AND PlaylistID = @PlaylistID)
                  )";
